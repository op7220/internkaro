...
class ApiService {
  ...
  static Future<LoginModel> login(BuildContext context,String email, String password) async {
    var responseBody;
    final response = await http.post(ApiConstant.getBaseURL(ApiConstant.LOGIN), body: {
  ...
  static Future<CitiesModel> getCities() async {
    var responseBody;
    final response =
        await http.get(ApiConstant.getBaseURL(ApiConstant.GET_CITIES));
    if (response.statusCode == 200) {
      print("response:${response.body}");
      responseBody = jsonDecode(response.body);
  ...
  static Future<SliderModel> getSlider() async {
    final response =
        await http.get(ApiConstant.getBaseURL(ApiConstant.GET_SLIDER_IMAGE));
    if (response.statusCode == 200) {
      print("response:${response.body}");
  ...
  static Future<CourseModel> getCourses() async {
    final response =
        await http.get(ApiConstant.getBaseURL(ApiConstant.GET_COURSE));
    if (response.statusCode == 200) {
      print("response:${response.body}");
      var responseBody = jsonDecode(response.body);
  ...
  static Future<JobListingModel> getPopularCategoriesList(
    ...
    final response = await http.post(
        ApiConstant.getBaseURL(ApiConstant.GET_POPULAR_CATEGORIES),
        body: {
          "location_url": selectedCity,
        });
    if (response.statusCode == 200) {
  ...
  static Future<DetailsScreenModel> getDetails(
    ...
    final response =
        await http.post(ApiConstant.getBaseURL(ApiConstant.GET_DETAILS), body: {
